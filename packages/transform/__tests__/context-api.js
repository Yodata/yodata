...
test("mapKeys only maps keys", () => {
  ...
  return expect(context.mapKeys({ "orange": "orange" })).toEqual({ "black": "orange" })
})
...
describe("fromYaml", () => {
  ...
  `
  test("fromYaml.parse", () => {
    return expect(Context.fromYaml(src)).toBeInstanceOf(Context)
  })
  test("fromYaml.context is functional", () => {
    return expect(Context.fromYaml(src).map({ "Foo": 1, "Bar": 1 })).toEqual({ foo: 1, bar: 1 })
  })
