...
function mapValueToContext(value: any, key: string, object: {}, context: {}): any {
  ...
  if (has(context, VALUE)) {
    switch (kindOf(context[VALUE])) {
      case "function":
        result = context[VALUE].call(this, { value, key, object, context })
        break
      case "object":
        result = Object.assign({}, context[VALUE])
        result = replaceValueTokens(result, {value, name: context[NAME], id: context[ID]})
        break
      case "string":
  ...
  if (has(context, "val") && kindOf(get(context, "val")) === "function") {
    result = invoke(context, "val", { value, key, last: object })
  }
