...
const applyChanges = object => {
   ...
   */
	...
	// Address.identifier => address.addressKey
	const nextAddress = contact.getIn(['address', '0']).set('addressKey', '_addressKey').delete('identifier')
	...
	const contactPoint = contact.getIn(['contactPoint', '0'])
		.delete('identifier')
   ...
   */
	const homeLocation = contact.getIn(['homeLocation', '0'])
		.delete('identifier')
		.set('phoneNumberKey', '_homePhoneNumberKey')
	contact = contact.setIn(['homeLocation', '0'], homeLocation)

	const workLocation = contact.getIn(['workLocation', '0'])
		.delete('identifier')
